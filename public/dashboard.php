...
async function loadData(){
 ...
 renderExport(exp);renderPreview(exp);
 if(window.lszjI18n)window.lszjI18n.translateAll();
}
function renderExport(exp){
 ...
 if(exp.can_export){
   cls='okbox';
   html+='<p>Alle freigegebenen, noch nicht exportierten Flüge sind exportbereit.</p>';
   html+='<p><a class="button ok" href="../src/export_vf_flights_csv.php?'+exportRangeQuery()+'">Flugdaten exportieren</a></p>';
   html+='<p><a class="button" href="../src/export_vf_members_csv.php">Personendaten exportieren</a></p>';
 } else {
   html+='<p>Keine exportbereiten Flüge oder Freigaben fehlen.</p>';
 }
 qs('exportBox').className='card '+cls;
 qs('exportBox').innerHTML=html;
}
