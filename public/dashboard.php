...
async function loadData(){
 try{
  ...
  const fl=await fetchJson('api_flight_approvals.php?'+rangeQuery()+'&status=all&_='+Date.now());
  const exp=await fetchJson('api_export_status.php?from='+encodeURIComponent(qs('from').value)+'&to='+encodeURIComponent(qs('to').value)+'&_='+Date.now());
