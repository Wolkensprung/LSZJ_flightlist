...
form.addEventListener('submit',async e=>{e.preventDefault();const roles=[...document.querySelectorAll('.ua-role')].map(x=>({code:x.dataset.code,enabled:x.querySelector('.enabled').checked,valid_from:x.querySelector('.valid-from').value,valid_until:x.querySelector('.valid-until').value}));const body={action:'save',csrf_token:csrf,user_id:Number(document.getElementById('userId').value),active:document.getElementById('active').checked,roles};const r=await fetch('../src/api_user_admin.php',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'same-origin',body:JSON.stringify(body)});const j=await r.json();if(!j.ok){showMessage(j.error,false);return}showMessage(j.message,true);render(j.user)});
})();
</script>
</body></html>
