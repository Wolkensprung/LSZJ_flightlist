...
$approvedNotExported=scalar_count($pdo,"SELECT COUNT(*) FROM accounting_entries WHERE $range AND approval_status='approved' AND exported_at IS NULL",$params);
...
$alreadyExported=scalar_count($pdo,"SELECT COUNT(*) FROM accounting_entries WHERE $range AND exported_at IS NOT NULL",$params);
...
$batch=$pdo->prepare("SELECT export_batch,COUNT(*) cnt,MIN(exported_at) first_exported_at,MAX(exported_at) last_exported_at FROM accounting_entries WHERE DATE(departure_time) BETWEEN ? AND ? AND exported_at IS NOT NULL GROUP BY export_batch ORDER BY last_exported_at DESC");$batch->execute($params);
