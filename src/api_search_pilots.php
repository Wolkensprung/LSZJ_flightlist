...
$pdo = db();
$sql = "SELECT vf_user_no, vf_member_no, display_name, email, mobile, priority_group
        FROM pilots_master
        WHERE is_active = 1
          AND is_selectable = 1";
if (!$all) {
...
$sql .= " AND (display_name LIKE ? OR search_name LIKE ? OR vf_user_no LIKE ?)
          ORDER BY is_primary DESC,
                   FIELD(priority_group, 'flying_member', 'student', 'gvvc', 'other'),
                   display_name
          LIMIT 30";

$stmt = $pdo->prepare($sql);
$searchKey = TextNormalizer::searchKey($q);
$stmt->execute(['%' . $q . '%', '%' . $searchKey . '%', '%' . $q . '%']);
